<!-- TOC -->
  ...
  * [Q-5 Whats the difference between Dirty Read, Non-Repeatable Read, Phantom Read](#q-5-whats-the-difference-between-dirty-read-non-repeatable-read-phantom-read)
...
Assume we have composite key on `Course` and `Date` column. 
The `CourseTitle` can be determined just from `Course` column that's why It's not in 2NF.
