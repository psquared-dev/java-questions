...
| **Key-Cleaner**     | Runs in the background, deleting old unused links and occasionally recycling their codes                                                                        |
...
(
    ...
    is_paid         BOOLEAN       NOT NULL DEFAULT FALSE, -- copied from users.plan at create time
    created_at      TIMESTAMPTZ   NOT NULL DEFAULT now(),
    last_visited_at TIMESTAMPTZ
);
...
### Why `is_paid` is stored on `url_mappings` instead of joined from `users`

Whether a link belongs to a paid user affects how a redirect behaves — most importantly, the two tiers expire on
completely different rules (§11): free links are deleted at a flat 6 months, while paid links survive until they've gone
2 years without a visit. The read path needs to know which tier a link is on.

The obvious way to get it would be to join `url_mappings` to `users` on each read and look at `users.plan`. But the read
path is the hottest path in the entire system (§9) — it runs ~100x more often than anything else — so making it do a
join on every click is exactly where you don't want extra work.

Instead, the plan is copied onto the link as a plain `is_paid` boolean at the moment the link is created (§7), and never
joined again on reads. This is a deliberate bit of **denormalization**: the same fact (this user is paid) now lives in
two places, `users.plan` and `url_mappings.is_paid`. The trade-offs:

- **Cost:** if a user upgrades or downgrades, their existing links keep the `is_paid` value from when they were created,
  unless a background job updates them. For this system that's fine — plan changes are rare, and slightly stale expiry
  behaviour on old links isn't harmful.
- **Benefit:** the read path becomes a single-table primary-key lookup with no join (§9), and any offline job that needs
  the plan (like Key-Cleaner in §11) can read whatever tables it needs, because it's not on the request path and nobody
  is waiting on it.

That's the recurring theme of this whole design: pay a little extra on the rare write to make the common read as cheap
as possible.

### Why the primary key is a `bigint id`, not the `code`

Both `url_mappings` and `keys_store` follow the same rule: a monotonic `bigint id` is the primary key, and `code` is a
separate unique index. Here's why.

It's tempting to make `code` the primary key — it's unique, and for `url_mappings` it's what every redirect looks up by.
But the `code` is deliberately scrambled and random-looking (§5), and that turns out to be a bad property for a primary
key.

Here's the mechanism. A B-tree index stays sorted by its key. When the key is monotonically increasing — like a
counter — every new row's key is larger than all the ones before it, so it always lands in the rightmost leaf of the
tree. That's the cheap, happy case: the tree grows on one edge and rarely has to reorganize. But when the key is random,
each new row lands in a *random* leaf somewhere in the middle of the tree. Those leaves are already full of existing
rows, so Postgres has to split pages to make room, and the tree needs far more frequent rebalancing. Write throughput
drops and the pages you touch are scattered instead of hot in cache.

This bites `keys_store` especially hard, because Key-Gen writes to it in bulk — 1,000 codes at a time (§5). If `code`
were the primary key, each bulk insert would scatter 1,000 random keys across the whole B-tree at once, which is close
to the worst case for page splits. Using a monotonic key as the primary key means those 1,000 rows append cleanly to the
right edge instead. For `keys_store` there's a natural one already: `key_id`, the pre-scramble sequence number the code
was generated from (§5), is itself monotonic and unique — so it serves as the primary key directly, no extra column
needed. For `url_mappings`, which has no such natural sequence column, a dedicated `bigint id` identity column plays the
same role.

A `bigint id` generated from a sequence is monotonic, so making *it* the primary key gives you one index that always
inserts on the right edge — the happy case. It also makes a cleaner internal handle: foreign keys and joins reference an
8-byte integer instead of a 7-character string.

**One honest caveat.** This does *not* make the random-insert cost disappear — it relocates it. Postgres tables are
heap-organized: the table's rows live in a heap, and *every* index (primary or not) is a separate structure pointing
into that heap. Since codes still have to be looked up by value — `url_mappings` on the redirect path, and both tables
need the unique index to guarantee no code is ever duplicated — there has to be an index on `code` no matter what, and
that index still receives random inserts with the same page-split cost. So what the `bigint` PK actually buys is:

- Each table now has *one* index taking random inserts (the `code` unique index) instead of that same random index
  *plus* a randomly-inserted primary key. The extra pain isn't doubled.
- A compact, monotonic, stable identifier for foreign keys, joins, and as the tiebreaker in cursor pagination.

The unavoidable part is that *some* index has to be keyed by `code`, because that's how codes are looked up and how
uniqueness is enforced — and that one will always take random inserts. The `bigint` PK just makes sure it's the *only*
one, and gives the rest of the schema a cheaper key to reference. (This is a Postgres-specific nuance: in an
index-organized engine like MySQL's InnoDB, where the table itself *is* the primary-key B-tree, the choice of primary
key matters even more, because a random PK would scatter the actual row storage, not just one index.)
...
sequenceDiagram
    ...
    MS ->> PG: INSERT INTO url_mappings(code, original_url, user_id, is_paid)
    alt insert fails
...
2. **Blocked-domain check.** The Gateway checks two things — is the host on the blocklist, and is the blocklist itself
   ...
   since it was last built. The next section explains why a freshness check is used here instead of a simple "is it
   loaded?" flag.
...
5. **Write the link.** The code, the long URL, and the user's plan are inserted into `url_mappings`. The `is_paid` value
   is copied from the user's current plan at this moment (§4), so the read path never has to look it up again:

   ```sql
   INSERT INTO url_mappings (code, original_url, user_id, is_paid)
   VALUES ($1, $2, $3, $4);
   ```
...
- Even if two Redis nodes somehow disagreed briefly, the unique index on `url_mappings.code` in Postgres (§4) is the
  final backstop against a code ever being double-assigned.
...
- **The read never joins `users`.** Everything a redirect needs — the destination URL and whether the link is paid — is
  already on the `url_mappings` row itself, because `is_paid` was copied there at create time (§4). This is the payoff
  of that denormalization: the single most frequent query in the system is a bare primary-key lookup on one table, which
  is about as cheap as a relational read gets.
- **The redirect is always a `302` (temporary), never a `301` (permanent).** This matters more than it sounds like it
  ...
  a link "working" in the browser even after we've recycled its code and reassigned it to someone else. `302` keeps
  every single click coming back through the Mapping Service.
...
sequenceDiagram
    ...
    R1 ->> PG: SELECT original_url, is_paid FROM url_mappings WHERE code = $1
...
Links that are long dead are wasted space. Key-Cleaner is a background job that periodically deletes them and returns
their codes to `keys_store` so they can be handed out again.
...
|------------------------------|--------------------------------------------------|-------------------------------------------------------|
| **Free** (`is_paid = FALSE`) | 6 months after `created_at`                      | **No** — a flat 6-month lifetime, visited or not      |
| **Paid** (`is_paid = TRUE`)  | 2 years after the last visit (`last_visited_at`) | **Yes** — only truly abandoned paid links are removed |
...
Because `is_paid` lives on the row itself (§4), each rule is a simple filter with no join to `users`. The two rules are
...
WHERE code IN (SELECT code
               FROM url_mappings
               WHERE is_paid = TRUE
                 AND COALESCE(last_visited_at, created_at) < now() - INTERVAL '2 years'
    FOR
UPDATE SKIP LOCKED
    LIMIT 1000
    )
...
### Returning the code to the pool

Both delete queries above end in `RETURNING code, key_id`, because deleting the mapping is only the first half of the
job — the freed code should go back into circulation for a future user. For each deleted row, Key-Cleaner then does two
more things: drop the now-stale cache entry, and re-insert the code into `keys_store` so Key-Sync can eventually hand it
out again.

```sql
-- Re-insert each freed code so it can be handed out again later.
INSERT INTO keys_store (key_id, code)
VALUES ($1, $2);
```

```
# Drop the stale redirect cache entry for each freed code.
DEL redirect:{code}
```

The order matters for the same reason it did in Key-Sync (§6): the mapping is deleted first (it's gone from
`url_mappings`), and only then is the code re-added to `keys_store`. If the job crashes in between, the code is simply
lost — a wasted code, which is fine (§5) — but it can never exist as both a live mapping and an available code at the
same time.
...
graph TB
    ...
    KS --> Redis
    KC -- " DELETE expired / INSERT recycled " --> PG
    KC --> Redis
    S3 --> GW
```
...
Round each of those up, then multiply by roughly 1.5–2x as headroom for bursts and slow-tail requests. Two rules of
thumb fall out of this:
...
- The fleet's total connection count must stay under Postgres's `max_connections`. Written as an inequality:

  ```
  (app instances × pool size per instance) + reserved slots  ≤  max_connections
  ```

  That left-hand sum, not CPU or memory, is usually the real ceiling. (The reserved slots cover superuser and
  maintenance connections Postgres keeps aside — see §13.6, where PgBouncer is introduced to keep the sum bounded as
...
Plugging in the numbers this system actually needs. From §13.3 the connection demand was ~3.5, but you'd provision a
pool with headroom — say a pool of 20 connections per box. Working the formula the direction you normally use it —
*given* a connection pool, how many cores back it:

```
cores ≈ (connections − effective_spindle_count) / 2
      ≈ (20            − 1)                       / 2
      ≈ 19 / 2
      ≈ 9.5   →  round to ~8-10 cores
```

Or flipping it around — *given* a core count, how many connections it supports:
...
Either direction lands in the same neighbourhood: a box with roughly 8 cores comfortably backs a pool of ~20
connections, which is far more than the ~3.5 of actual demand — the extra is the burst-and-tail headroom from §13.3.

### 13.5 Picking an actual machine

Putting §13.3 and §13.4 together, something in the shape of "8 cores / ~20 connections / ~5,000 IOPS" is a reasonable
target — which maps to roughly a `db.r6g.2xlarge` (8 vCPU, 64GB RAM) on a gp3 volume provisioned for 5,000 IOPS. RAM
matters here mainly because Postgres wants to keep its hot rows and indexes in memory.
...
```
writes (2.5) + misses (~1.0) ≈ 3.5 connections
→ round up, then apply headroom once, to this combined total
```
...
Table size is estimated as `bytes per row × number of rows × 3` — the ×3 accounts for indexes and general storage
overhead. The byte estimates per column: `bigint` = 8, the 7-character `code` ≈ 7, `original_url` ≈ 100 (this is the
biggest single contributor), `uuid` = 16, each `timestamptz` = 8.

| Table                                                               | Row size   | Raw size at 79B rows | With ×3 overhead | Fits on one machine?                                                                                                                |
|---------------------------------------------------------------------|------------|----------------------|------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| `keys_store` (code + key_id)                                        | ~15 bytes  | ~1.18 TB             | ~3.5 TB          | **Yes.** It's off the hot path — only Key-Sync reads it — and it's a bounded buffer that actually shrinks as codes move into Redis. |
| `url_mappings` (id + code + url + user_id + is_paid + 2 timestamps) | ~150 bytes | ~12 TB               | ~35 TB           | **No.** This one needs to be split across machines.                                                                                 |

### 14.3 Splitting `url_mappings` across machines

At ~35TB, `url_mappings` is well past what one Postgres box should hold, so it gets split by `hash(code)` into roughly
**20 shards** (35TB ÷ ~2TB of comfortably usable space per shard).
...
2. **V2 — add safety mechanisms before real traffic.** The cache-stampede lock (§10) and Key-Cleaner with its recycling
   rule (§11) should both be in place before this sees production load.
