...
**Translating "8 vCPU" into actual cores.** The `db.r6g.2xlarge` advertises "8 vCPU," but that's not 8 physical cores. A
cloud vCPU is normally one *hyperthread* — a physical core presents itself to the OS as two logical cores, so the
advertised vCPU count is roughly double the physical cores. To get real cores, halve it:

```
physical cores ≈ vCPU / 2
               = 8 / 2
               = 4 physical cores
```

The Postgres formula in §13.4 is expressed in *physical* cores (it's estimating real compute), so compare like with
like:

```
need (from §13.4):   ~2-3 physical cores
have (8 vCPU / 2):    4 physical cores    ✓  comfortably enough
```

So there's no contradiction between "we calculated ~2-3 cores" and "the box has 8 vCPU" — once you undo the hyperthread
doubling, 8 vCPU is ~4 physical cores, which is a bit more than the write workload needs. And the exact margin barely
matters here, because CPU isn't why this box was chosen — the RAM and IOPS were.
