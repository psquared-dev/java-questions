...
Claim & commit successful
       │
       ├─► [pays in time] ──► status becomes `payment_done` (§10)
       │
       └─► [walks away]   ──► expiry job frees the seats     (§11)
```
...
A retried `POST /checkout` passes through both guards in turn. First it answers "which booking?" using the frontend key.
Then it answers "which page?" using the Stripe key. Neither guard alone is enough; the two together are what make the
retry safe.
...
sequenceDiagram
    ...
    U ->> BK: POST /bookings/900/checkout (carries frontend idem_abc)
    BK ->> PG: find booking by (user_77, idem_abc) → booking 900
    Note over BK: status=payment_pending, payment_id=NULL → must create page
    BK ->> S: POST /checkout/sessions (Idempotency-Key: booking-900-1, amount 45000)
...
1. Browser calls `POST /bookings/900/checkout`, carrying the **frontend** `idempotency_key` (`idem_abc`).
...
3. Backend derives the **Stripe** key `booking-900-1` (from `booking_id = 900` and `payment_attempt = 1`) and calls
   Stripe with it and `amount = 45000`.
...
Every call re-derives what to do from the booking's `status` and `payment_id`. The one rule that keeps this correct:
**to learn a page's liveness, do a live `GET` on the session id — never rely on re-POSTing the create call to tell you**
...
- `payment_id` already set → **`GET` that session from Stripe** and read its live `status`:

| Live session `status` (from GET) | Meaning                   | Action                                                                                                                |
|----------------------------------|---------------------------|-----------------------------------------------------------------------------------------------------------------------|
| `open`                           | still payable             | return the **same** `url`                                                                                             |
| `complete`                       | paid                      | mark our booking `payment_done`                                                                                       |
| `expired`                        | url is dead (`url: null`) | bump `payment_attempt`, create a **new** session with the new derived key, overwrite `payment_id`, return the new url |
...
┌─ POST /checkout/sessions,  Idempotency-Key: booking-900-1 ───────────┐
...
┌─ GET /checkout/sessions/cs_55  (look up the session by id) ──────────┐
...
(`GET /checkout/sessions/cs_55`) — that reads the session's live status. The POST-with-idempotency-key is only for
...
| Know if a page is still alive   | `GET /checkout/sessions/cs_55`                               | it reads current status, not a frozen recording                |
...
```text
t0  Browser → POST /checkout  (carries frontend idem_abc)
...
t4  Browser times out, retries → POST /checkout again (SAME frontend idem_abc)
...
```text
Browser → POST /checkout (again, same idem_abc)
...
```text
Browser → POST /checkout (later, same idem_abc)
...
### 14.1 The number everything derives from
...
### 14.2 How long does one booking hold a database connection?
...
### 14.3 Turning that into connections — Little's Law
...
### 14.4 From connections to cores
...
### 14.6 The connection-pool inequality
...
### 14.8 The reassurance that makes all this safe
...
"paid," with a poll-based backup and a helpline that obey the identical transitions so they can never disagree. Scaling
comes last and in order: one app + one Postgres with the unique insert, then Redis read-through, then more app copies
(the unique insert still picks one winner across all of them), then Hikari pools sized so
`instances × pool ≤ max_connections`, then load-testing to replace the ~20ms-per-booking guess — with the worked example
that at 1,000 bookings/sec a ~20ms booking means one connection does 50/sec so you need ~20 busy connections, ~8-10
cores (a `db.r6g.2xlarge`) as a starting guess, and the closing reassurance that all 1,000 people fighting for one seat
still produce only one success, because the unique key is doing its job.