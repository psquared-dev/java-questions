...
8. [The life of a message — tap to blue tick](#8-the-life-of-a-message-tap-to-blue-tick-same-server)
...
12. [Offline delivery — the sync cursor](#12-offline-delivery--the-sync-cursor)
13. [Receipts — delivered and read (the ticks)](#13-receipts--delivered-and-read-the-ticks)
14. [Staying alive — heartbeats and dead sockets](#14-staying-alive--heartbeats-and-dead-sockets)
...
erDiagram
    ...
    USERS ||--o{ CONVERSATION_PARTICIPANTS: cursor
...
(
    ...
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now(), -- bumped on each new message, for sync (§12)
    -- store the pair with user_a < user_b, so one pair is stored exactly once
    UNIQUE (user_a, user_b),
    CHECK (user_a < user_b)
);

-- Find "my conversations that changed since I last synced" (§12), for both participants.
CREATE INDEX idx_conversations_a_updated ON conversations (user_a, updated_at);
CREATE INDEX idx_conversations_b_updated ON conversations (user_b, updated_at);
...
-- Per-participant read/delivered cursors (the "high-water marks" for ticks, §13).
-- One tiny row per (conversation, user) — NOT one row per message.
CREATE TABLE conversation_participants
(
    conversation_id    BIGINT NOT NULL REFERENCES conversations (id),
    user_id            BIGINT NOT NULL REFERENCES users (id),
    last_delivered_seq BIGINT NOT NULL DEFAULT 0, -- reached this user's device
    last_read_seq      BIGINT NOT NULL DEFAULT 0, -- this user opened & saw it
    PRIMARY KEY (conversation_id, user_id)
);
```

### What each table holds, column by column

Before the takeaways, here's what the important columns do in each server-side table.

**`users`** — one row per person.

- `id` — the internal user id everything references (the `from`/`to` on the wire, §6).
- `username` — the unique handle people log in / search by (`UNIQUE`).
- `last_seen` — when they were last active, for the "last seen" display.

**`conversations`** — one row per *pair* of people (1:1).

- `user_a`, `user_b` — the two participants, always stored smaller-id-first (`CHECK (user_a < user_b)`), so a pair maps
  to exactly one row no matter who started the chat.
- `last_seq` — the per-conversation message counter. Every new message bumps it and takes the new value as its `seq`
  (§10). This is the field that hands out message order.
- `updated_at` — bumped on every new message, so a reconnecting phone can ask "which of my conversations changed since I
  last synced?" in one query (§12).

**`messages`** — one row per message (the durable history; this is the huge table).

- `id` — the global message id (a surrogate key).
- `conversation_id` — which chat this belongs to.
- `sender_id` — who sent it (this is where the wire field `from` lands, §6).
- `seq` — the message's position *within its conversation* (1, 2, 3, …). This is what orders the chat and what the
  catch-up cursor compares against (§10, §12).
- `body` — the message text.
- `client_msg_id` — the id the *phone* generated before sending, used to dedup retries (§11).
- `created_at` — wall-clock time, for display only — never for ordering (clocks drift; two messages can share a
  timestamp).

**`conversation_participants`** — one small row per (conversation, user); the receipt cursors.

- `last_delivered_seq` — the highest `seq` that reached this user's device. Drives the two-grey-ticks state (§13).
- `last_read_seq` — the highest `seq` this user has actually opened and seen. Drives the blue-ticks state (§13).
- These are *high-water marks*: "read up to seq N" means everything ≤ N is read, so one number replaces a flag on every
  message.

Four things worth calling out, because the rest of the doc depends on them:

- **One conversation row per pair, stored once.** The `CHECK (user_a < user_b)` plus `UNIQUE(user_a, user_b)` means
  (Aisha, Ben) and (Ben, Aisha) resolve to the *same* row. You must always order the pair smaller-id-first *before*
  inserting — regardless of who is sending — or you'd break this rule (§8 shows the `LEAST`/`GREATEST` ordering).
...
- **Read/delivered receipts are cursors, not per-message flags.** `conversation_participants` holds *one number per
  person per conversation* ("read up to seq N"), never a status column on each of the billions of `messages` rows (§13).
  Advancing a receipt is a single-row update, not a rewrite of the message history.

### The phone's own database (SQLite)

The tables above live on the **server**. The phone keeps its *own* small database (SQLite) — a local mirror so the UI
works instantly and offline, reconciling with the server as acks arrive. Two tables matter:

```sql
-- On the PHONE (SQLite). A local mirror, not the source of truth.

CREATE TABLE local_conversations
(
    conversation_id   INTEGER,                    -- NULL until the server assigns it
    peer_user_id      INTEGER NOT NULL,           -- the other person
    peer_username     TEXT,
    last_received_seq INTEGER NOT NULL DEFAULT 0, -- my catch-up cursor (§12)
    last_read_seq     INTEGER NOT NULL DEFAULT 0, -- how far I've read
    updated_at        TEXT
);

CREATE TABLE local_messages
(
    id              INTEGER,          -- server's message id; NULL until acked
    conversation_id INTEGER,          -- NULL until the server assigns it
    sender_id       INTEGER NOT NULL,
    seq             INTEGER,          -- NULL until the server assigns it (§10)
    body            TEXT    NOT NULL,
    client_msg_id   TEXT    NOT NULL, -- generated on THIS phone before sending (§11)
    status          TEXT    NOT NULL, -- 'pending'|'sent'|'delivered'|'read'  → drives the ticks
    created_at      TEXT
);
```

The two phone tables mirror the server's, but with a few phone-specific columns:

**`local_conversations`** — the phone's copy of each chat it knows about.

- `conversation_id` — the server's id for this chat; NULL for a brand-new chat until the server assigns it in the first
  ack.
- `peer_user_id`, `peer_username` — the other person (the phone stores the peer directly, since a 1:1 chat is "me and
  them").
- `last_received_seq` — a bookmark that says "I already have every message in this chat up to here." So when the phone
  reconnects after being offline, instead of re-downloading the whole conversation, it tells the server this number and
  gets back only the messages that came after it — exactly what it missed, nothing more (§12).
- `last_read_seq` — how far *this user* has read in the chat (used to send read receipts, §13).

**`local_messages`** — the phone's copy of the messages it has, plus its own send-state.

- `id`, `conversation_id`, `seq` — all NULL for a just-typed outgoing message, because only the *server* assigns them;
  the phone fills them in from the ack.
- `sender_id`, `body`, `created_at` — who/what/when, as shown in the bubble.
- `client_msg_id` — generated on *this* phone before sending, so a retry dedups to the same server row (§11).
- `status` — the phone-only lifecycle field that drives the ticks: `pending` = clock icon, `sent` = one grey tick,
  `delivered` = two grey, `read` = two blue.

A freshly-typed message starts `pending` with `seq`, `id`, and `conversation_id` all NULL, and the phone fills them in
as acks arrive. This whole lifecycle is walked through end-to-end in §8.
...
The server is not busy-looping over its sockets. Its I/O threads are asleep in the OS, waiting (this is what `epoll`does
on Linux). The OS watches all the open sockets. When bytes land on Aisha's socket, the OS **wakes an I/O thread**and
tells it "this socket has data." A silent socket costs almost nothing — the CPU is only spent when real data actually
arrives.
...
The registry entry has a **TTL** and is refreshed by heartbeat (§14), so it points at a live server, not a dead one. The
...
## 8. The life of a message — tap to blue tick (same server)

### The idea

This is the **full life of a message**, end to end: Aisha taps send, and we follow it all the way to the blue tick on
her screen — through her phone's local database, the store-then-deliver rule on the server, Ben's device, and back. We
use the simplest topology (both people on the *same* chat server, no router) so the mechanics are clear; §9 adds the
router hop for the cross-server case.
...
sequenceDiagram
    ...
    participant Ben
    Note over Aisha: tap send → write local SQLite (status=pending, clock icon)
    Aisha ->> A: { to: 9, from: 7 (from stamp), body: "hi", client_msg_id: "abc" }
    A ->> PG: BEGIN, find/create conversation, bump last_seq, INSERT message, COMMIT
    PG -->> A: seq = 5
    A -->> Aisha: ack { client_msg_id: "abc", seq: 5 }
    Note over Aisha: match client_msg_id → status=sent (one grey tick)
    A ->> A: local lookup user 9 → Channel
    A ->> Ben: deliver { from: 7, seq: 5, body: "hi" }
    Note over Ben: store locally (status=delivered), last_received_seq=5
    Ben -->> A: delivered-ack { conversation_id: 12, seq: 5 }
    A -->> Aisha: delivered up to 5
    Note over Aisha: status=delivered (two grey ticks)
    Note over Ben: opens chat, message in viewport → read
    Ben -->> A: read_receipt { conversation_id: 12, last_read_seq: 5 }
    A -->> Aisha: read up to 5
    Note over Aisha: status=read (two BLUE ticks)
```

*Fig. 2 — The whole arc: tap → stored → sent → delivered → read. `seq` (assigned at store time) is the thread running
through every step.*

Walking through it at the level of every wake-up, read, lookup, and write:

**Step 0 — Aisha taps send (phone-local, before any network).** Her phone captures the message in its own SQLite
database *first*, so the UI is instant and survives a dead network. It generates a `client_msg_id` for dedup (§11), and
writes two rows — note that `seq`, the server `id`, and even `conversation_id` are all still NULL, because only the
*server* assigns those:

```sql
-- On Aisha's PHONE (SQLite), the instant she taps send:

INSERT INTO local_conversations (conversation_id, peer_user_id, peer_username,
                                 last_received_seq, last_read_seq, updated_at)
VALUES (NULL, 9, 'Ben', 0, 0, '2026-09-13 20:00:00'); -- new chat, not yet server-assigned

INSERT INTO local_messages (id, conversation_id, sender_id, seq, body,
                            client_msg_id, status, created_at)
VALUES (NULL, NULL, 7, NULL, 'hi', 'abc', 'pending', '2026-09-13 20:00:00');
```

The `status = 'pending'` renders a **clock icon** on Aisha's bubble. Only now does the phone put the message on the
wire.

**Step 1 — bytes arrive at Server A, epoll wakes a thread.** Aisha's phone writes the frame down its socket to Server A.
The bytes land in Server A's receive buffer. The OS marks that socket "ready" and epoll wakes an I/O thread on Server A:
"this Channel has data." The frame the phone sent:
...
Note it carries `to` but **not** `from` — the server won't trust the phone for identity (§6).

**Step 2 — read `from` from the stamp, `to` from the payload.** The thread has the `Channel` but not the identity, so it
reads the stamp:

```text
from = channel.attr(USER_ID).get();   // → 7  (trusted; set at login, §6)
```

So `from = 7` (from the stamp), and `to = 9`, `body = "hi"`, `client_msg_id = "abc"` (from the payload).

**Step 3 — store, in ONE transaction.** This is the golden rule — **store, then deliver** — and the three writes below
must be a *single* transaction. Server A runs:

```sql
BEGIN;

-- The pair is ALWAYS ordered smaller-id-first before use, regardless of who
-- is sending, so (from=7, to=9) and (from=9, to=7) resolve to the SAME row.
--   user_a = LEAST(7, 9) = 7 ;  user_b = GREATEST(7, 9) = 9
-- (Here 7<9 already; if Aisha were user 9 sending to Ben user 7, we'd still
--  write user_a=7, user_b=9 — never a second row for the same pair.)

-- (a) Find or create the conversation row for the pair.
INSERT INTO conversations (user_a, user_b)
VALUES (7, 9) -- LEAST first, GREATEST second
    ON CONFLICT (user_a, user_b) DO NOTHING;

-- (b) Bump this conversation's counter and read back the new value.
--     That returned number IS this message's seq (§10). Also bump updated_at
--     in the SAME statement, so the sync marker (§12) can never drift.
UPDATE conversations
SET last_seq   = last_seq + 1,
    updated_at = now()
WHERE user_a = 7
  AND user_b = 9 RETURNING id, last_seq;
-- → conversation_id = 12, seq = 5

-- (c) Insert the message with that seq. "from" (7) goes into sender_id.
...
**Why order the pair smaller-id-first?** If we just put the *sender* in `user_a`, then John→Paul would create
`(john, paul)` and Paul→John would create `(paul, john)` — two rows for one conversation, and one of them would violate
`CHECK (user_a < user_b)` (§4). Ordering the ids before the insert means the same pair always maps to one row, no matter
who sends first.

**Why one transaction?** Steps (b) and (c) must be atomic. If the counter bump committed but the message insert didn't,
we'd have burned `seq = 5` with no message at 5 — a permanent gap. And if two of Aisha's messages ran their bumps
interleaved without the transaction serializing them, they could both read `last_seq = 4` and both try to write
`seq = 5` — a collision. The `UPDATE` in (b) takes a row lock on the conversation, so a second concurrent send waits for
the first to commit before it can bump — that's what serializes the two `seq` grabs. The `UNIQUE(conversation_id, seq)`
constraint (§4) is the final guard behind it.

**Step 4 — the store hands back `seq`, and the sender is acked with it.** The `RETURNING` in step (b) is where `seq = 5`
is produced. Server A sends it straight back to Aisha:
...
Aisha's phone now reconciles its **local SQLite**, which is *why* the server returns these ids — the pending row was
created with them all NULL (step 0), and now they get filled in:

```sql
-- On Aisha's PHONE: match the ack to the pending row by client_msg_id, fill in
-- the server-assigned ids, and flip the status → one grey tick.
UPDATE local_messages
SET id              = 849201,
    conversation_id = 12,
    seq             = 5,
    status          = 'sent'
WHERE client_msg_id = 'abc';

UPDATE local_conversations
SET conversation_id   = 12,
    last_received_seq = 5 -- she "has" her own message
WHERE peer_user_id = 9;
```

So the `seq` is used three ways here: it **confirms** the message (match on `client_msg_id`, clock → one grey tick), it
gives the message its **position** in the chat (for ordering), and it **advances Aisha's own cursor** so she won't
re-fetch her own message on reconnect (§12).

**Step 5 — local lookup and deliver.** Server A looks up `user 9 → Channel` in its *local* map (§7). Ben is on the same
server, so the lookup finds his pipe, and Server A writes the message down it:

```text
deliver → Ben: { from: 7, seq: 5, body: "hi" }
```

Note `from: 7` and `seq: 5` travel *to Ben*, not just the body — Ben's phone needs `from` to file the message under the
right conversation, and `seq` to place it in order and advance *his* cursor (§12). No registry, no router needed here —
the recipient happened to be on the same server as the sender.

**Step 6 — Ben's phone receives, stores locally, and sends a delivered ack.** Bytes arrive on Ben's phone (epoll wakes a
thread there). His app writes its **own SQLite** — this is a brand-new chat for Ben, so it creates the local
conversation *and* the message, both at `delivered`:

```sql
-- On Ben's PHONE (SQLite):

INSERT INTO local_conversations (conversation_id, peer_user_id, peer_username,
                                 last_received_seq, last_read_seq, updated_at)
VALUES (12, 7, 'Aisha', 5, 0, '2026-09-13 20:00:02'); -- synced to seq 5

INSERT INTO local_messages (id, conversation_id, sender_id, seq, body,
                            client_msg_id, status, created_at)
VALUES (849201, 12, 7, 5, 'hi', 'abc', 'delivered', '2026-09-13 20:00:02');
```

Then Ben's phone sends a **delivered ack** back up its socket — a small frame naming exactly which message arrived:
...
`conversation_id` + `seq` is enough to identify the exact message — that's another thing `seq` is for.

**Step 7 — the delivered ack advances a cursor and reaches Aisha (two grey ticks).** Server A receives the ack and does
two things — the same two-lane pattern used for read receipts (§13):

- **Durable:** advance Ben's *delivered* cursor for this conversation — a one-row update, not a rewrite of the message:

  ```sql
  UPDATE conversation_participants
  SET last_delivered_seq = GREATEST(last_delivered_seq, 5)
  WHERE conversation_id = 12 AND user_id = 9;
  ```

- **Fast:** Server A already holds Aisha's `Channel` (she's on this server), so it pushes "delivered up to 5" down her
  socket. Aisha's phone updates SQLite and shows **two grey ticks**:

  ```sql
  -- On Aisha's PHONE:
  UPDATE local_messages SET status = 'delivered'
  WHERE conversation_id = 12 AND seq <= 5 AND sender_id = 7;
  ```

  (If Aisha were offline, she'd pick this up from the cursor on reconnect — §14 for how offline sync reads peer
  cursors.)

**Step 8 — Ben reads it (two blue ticks).** Later Ben opens the chat. The message scrolls into his viewport with the app
focused, so it counts as **read**. His phone debounces (~1s, §13), updates its own `last_read_seq`, and sends one
coalesced read receipt:
...
Server C-or-A (here, the same server) again does the two lanes (§13):

- **Durable:**
  `UPDATE conversation_participants SET last_read_seq = GREATEST(last_read_seq, 5) WHERE conversation_id = 12 AND user_id = 9;`
- **Fast:** push "read up to 5" to Aisha. Her phone flips the bubbles blue:
...
And that's the full arc: `pending → sent → delivered → read`, driven end to end by `seq` and reconciled on both phones'
local databases. This is the same-server, fast case. The next section handles the general case, where Ben is on a
*different* server — the same steps, with a router hop added in the middle.
...
Storing is exactly the same single transaction as the same-server case (§8, step 3): find-or-create the conversation,
bump its counter to get `seq`, insert the message — all atomic.

```sql
BEGIN;
INSERT INTO conversations (user_a, user_b)
VALUES (7, 9) ON CONFLICT (user_a, user_b) DO NOTHING;
UPDATE conversations
SET last_seq   = last_seq + 1,
    updated_at = now()
WHERE user_a = 7
  AND user_b = 9 RETURNING id, last_seq; -- → conversation_id = 12, seq = 5
INSERT INTO messages (conversation_id, sender_id, seq, body, client_msg_id)
VALUES (12, 7, 5, 'hi', 'abc');
COMMIT;
```
...
```text
5. Store the message in the ONE transaction above → seq = 5 comes back.
   Server A acks Aisha with { client_msg_id: "abc", seq: 5 } → her app shows
   one grey tick and advances her cursor to seq 5 (§8 explains the seq uses).
   (Store BEFORE delivering — §8.)
6. Server A checks its OWN local map for user 9. Ben is NOT on Server A,
...
```text
7. Server A picks a router by hashing the user id (any router works; every
   router links every server). Say it picks Router 1.
8. Server A writes the ENVELOPE { to: 9, from: 7, body: "hi", seq: 5, ... }
   down its socket to Router 1.  ← note: it did NOT look up where Ben is;
   that's the router's job.
9. The bytes land on Router 1. epoll WAKES a thread on Router 1: "the socket
...
```text
10. Router 1 reads the envelope and sees  to: 9.
...
11. Router 1 looks up user 9 in the REGISTRY (Redis):  conn:9 → "Server C".
    This is the single Redis read in the whole trip.
12. Router 1 turns the name "Server C" into a live link using its own local
...
```text
13. Router 1 writes the same envelope { to: 9, ... } down its link to Server C.
14. The bytes land on Server C. epoll WAKES a thread on Server C: "the socket
    from Router 1 has data."
15. Server C reads the envelope and sees  to: 9.
    → Again, NO stamp is read: this socket's other end is Router 1, not Ben.
      The recipient is taken from the envelope's  to  field.
16. Server C looks up user 9 in ITS OWN local map ( user → Channel ) → Ben's pipe.
17. Server C writes "hi" down Ben's phone socket → Ben's app receives it.
```

**The ack travels the same path in reverse**

```text
18. Ben's app sends a "delivered" ack. It lands on Server C (epoll wake),
...
| Router 1, step 10 | a **chat-server** socket | the **envelope**'s `to` field (9)        |
| Server C, step 15 | a **router** socket      | the **envelope**'s `to` field (9)        |
...
- **Two server-to-server hops, but only one registry lookup.** Between chat servers the message crosses two links —
  ...
  tier. But Redis is read **exactly once** across the whole trip — by the router (step 11), not by Server A and not by
...
is already in the database (step 5). The receiver reconnects to some server, the registry updates, and the phone catches
...
## 12. Offline delivery — the sync cursor

### The idea

While Ben is offline, messages for him pile up in the database (already stored — §8). When he reconnects, he must pull
*exactly* what he missed — no more, no less — without re-downloading his whole history. The mechanism is a
per-conversation **cursor**: for each chat, Ben's phone remembers the `seq` of the last message it has. Catching up is
then "give me everything after that."

### The client's local state

Ben's phone keeps one number per conversation:

```text
conversation_id → last_received_seq
```

For example, after some earlier chatting:

```text
conversation 10 → last_received_seq = 50
conversation 12 → last_received_seq = 40
```

This is the cursor. It says, per chat, "I already have everything up to here."

### Two different things to sync

When Ben reconnects there are **two** distinct problems, and it's worth keeping them apart:

1. **Conversations Ben already knows about, that got new messages** while he was away.
2. **Brand-new conversations** that were started while he was away — chats his phone has never heard of.

Let's do each with a concrete example.

**Case 1 — a known conversation with new messages.**

Ben's phone knows conversation 10 and has up to `seq 50`. Meanwhile the server's row says:

```text
conversations
id | user_a | user_b | last_seq
---|--------|--------|---------
10 | ...    | Ben    | 55
```

The server's `last_seq = 55` is ahead of Ben's `50`, so Ben missed 51–55. His phone asks for the gap:

```sql
SELECT id, sender_id, seq, body, created_at
FROM messages
WHERE conversation_id = 10
  AND seq > 50 -- Ben's last_received_seq for this chat
ORDER BY seq LIMIT 50; -- page in chunks if there are many
```

The server returns seq 51, 52, 53, 54, 55. Ben stores them and advances his cursor:

```text
conversation 10 → last_received_seq = 55
```

**Case 2 — a brand-new conversation.**

While Ben was offline, Aisha started a *new* chat with him:
...
Ben's phone has **never seen conversation 20** — there's no cursor for it at all. So before it can pull messages, it
must first *discover that 20 exists and involves Ben*. Once it knows, it initializes the cursor at 0 and pulls from the
start:

```text
conversation 20 → last_received_seq = 0
```
...
The server returns seq 1 and 2; Ben stores them and sets `conversation 20 → last_received_seq = 2`.

### How Ben discovers what changed — without downloading everything

Case 1 and Case 2 both start with the same question: *"which of my conversations have anything new since I was last
here?"* The naive answer — "download the whole `conversations` table for Ben and compare each one" — is wasteful when
Ben has 300 chats and only 2 moved. The fix is the `updated_at` column on `conversations` (§4), which is bumped every
time a message is stored in that conversation — in the same transaction as the message insert (§8), so it can never
drift.

Ben's phone keeps a single "last synced at" timestamp. On reconnect it asks one question:

```sql
-- "Which of my conversations changed since I last synced?"
SELECT id, user_a, user_b, last_seq, updated_at
FROM conversations
WHERE (user_a = :ben OR user_b = :ben)
  AND updated_at > :last_synced_at
ORDER BY updated_at;
```

This one query returns **both** cases at once:

- A row Ben's phone *already knows* (conversation 10) with `last_seq` ahead of his cursor → Case 1, fetch the gap.
- A row Ben's phone has *never seen* (conversation 20) → Case 2, add it locally at cursor 0 and fetch from seq 1.

Then Ben advances his "last synced at" to the newest `updated_at` he saw. Next reconnect only looks at what changed
*after* that — so the cost scales with what actually moved, not with how many chats Ben has.
...
Ben reconnects
      │
      ▼
Ask: conversations where (I'm a participant) AND updated_at > last_synced_at
      │
      ├── row I already know, server last_seq > my cursor
      │        → fetch messages WHERE seq > my_cursor  (Case 1)
      │        → advance my cursor
      │
      └── row I've never seen
               → add it locally at cursor 0            (Case 2)
               → fetch messages WHERE seq > 0
               → set my cursor
      │
      ▼
advance "last synced at" → live pushes take over on the open socket
```

The catch-up pull is one-time — it fills the gap, then the phone goes back to receiving live pushes on the open socket
(§8). The database *is* the offline queue; the cursor is just a bookmark into it. (An explicit Redis offline queue is a
possible optimization — faster to read, but a second store to keep in sync — so V1 keeps the cursor over the database.)

### Sending while offline

The mirror image: if Aisha composes a message with no connection, her phone saves it in a local **outbox** with its
`client_msg_id`, shown as pending (a clock icon). On reconnect the outbox flushes; the server stores it, assigns `seq`,
and acks — and the clock becomes a tick.

The key subtlety is *when `seq` is assigned*: it's set when the message **reaches the server**, not when it was typed.
So an offline-composed message slots into order *at arrival*, landing after whatever reached the server first. If both
people are offline, the message simply waits in whichever store has it — Aisha's outbox, or the database once it's
stored — until each side comes back. It's never lost, because at every moment it sits safely *somewhere*.

---

## 13. Receipts — delivered and read (the ticks)

### The idea

Aisha wants her ticks to update: one grey (sent/stored), two grey (delivered to Ben's device), two blue (Ben opened and
read it). The naive way — mark each `messages` row `delivered` then `read` — is a write per message, and at ~79 billion
messages that write amplification would crush the database. The scalable model is a **high-water-mark cursor**: store
*one number per person per conversation*, not a flag per message.

### The cursor model — one number, not many flags

`conversation_participants` (§4) holds two numbers per (conversation, user):
...
`last_read_seq = 5` *means* "Ben has read messages 1 through 5." You don't mark five rows — you set one number to 5. If
Ben then reads up to seq 500, that's not 495 more writes; it's one number moving from 5 to 500. This is the whole
scaling trick: **O (1) pointer update, not O (N) row updates.**

Rendering Aisha's ticks is then pure comparison — no per-message state:

```text
for each message Aisha SENT in conversation 12 (she knows its seq):
   seq ≤ last_read_seq[Ben]      → two BLUE ticks
   seq ≤ last_delivered_seq[Ben] → two grey ticks
   otherwise (just stored)       → one grey tick
```

### The client side — viewport + debounce

Ben's phone decides "read" from what's actually **on screen**: when a message scrolls into the viewport and the app is
focused, it counts as read. But Ben scrolls fast — seq 5, 6, 7, 8 flash past in a second — so the phone does **not**fire
a receipt per message. It **debounces**: it waits until scrolling settles (~1s), takes the highest seq seen, and sends
**one** coalesced frame:
...
So reading 50 messages is one small frame carrying `last_read_seq: 50`, not 50 frames. (The phone sends this to the
**server** over its socket — phones never write to the database directly, §8.)

### The server side — two lanes

When the receipt reaches Ben's chat server (say Server C), it does two independent things — the same two-lane split used
for the delivered ack in §8:

**Lane 1 — durable cursor (always).** Advance the number, idempotently:

```sql
UPDATE conversation_participants
SET last_read_seq = GREATEST(last_read_seq, 8)
WHERE conversation_id = 12
  AND user_id = 9;
```

`GREATEST` makes it monotonic and idempotent — a re-sent "read up to 8", or an out-of-order "read up to 5" arriving
after 8, can never move the cursor backward. So at-least-once delivery (§11) is fine here: duplicates and reordering are
harmless.

**Lane 2 — live push (best-effort).** Notify Aisha *now* if she's online. Server C finds Aisha exactly the way it finds
any recipient (§9): look her up in the registry (`user → server`), forward through a router to her chat server, which
pushes down her socket:
...
Aisha's app then flips her sent bubbles with `seq ≤ 8` to blue. If Aisha is **offline**, Lane 2 simply finds no live
socket and does nothing — and that's fine, because Lane 1 already stored the truth. She picks it up on reconnect
(below).

### Scaling the durable write — Redis-hot, Postgres-cold

Even one row-update per receipt can be a lot under a flash crowd. Because a *lost recent receipt is harmless* — the
worst case is a tick stays grey a little longer, never a wrong or lost message — you can keep the hot cursor in Redis
and flush to Postgres in batches:

```text
on read receipt:
  1. Redis:  HSET read:12  9  8        ← O(1), in-memory (the hot path)
  2. Lane 2 push to Aisha if online
  3. (background) every few seconds, a worker flushes changed cursors
     Redis → Postgres in ONE batched write
```

So the hot path is a memory op plus a maybe-push — no synchronous Postgres write at all. Postgres receives a batched
trickle off the hot path. This is the "graceful degradation" lever too: under extreme load you can throttle or delay the
receipt queue to protect core message delivery, because receipts tolerate lag and messages don't.

### Offline Aisha — receipts sync like messages

If Aisha was offline when Ben read, she learns the blue ticks the same way she learns missed messages (§12). Her
reconnect sync already asks "which of my conversations changed?" — for each one, she also reads the *other* party's
cursors:

```sql
-- On reconnect, for a conversation Aisha is catching up on:
SELECT user_id, last_delivered_seq, last_read_seq
FROM conversation_participants
WHERE conversation_id = 12
  AND user_id = 9; -- the peer (Ben)
```

She then colors every sent bubble by comparing its `seq` to those two numbers. One read per conversation, no per-message
scan.

### Why this is all 1-on-1

Everything above assumes exactly **one** other participant, so a conversation has two `conversation_participants` rows
and a receipt is a single peer's cursor. Group chats are a different problem — N participants means N cursors per
conversation and a potential N×M receipt storm, usually handled with heavier client aggregation ("read by 12") — but
this design is scoped to 1-on-1 (§1), so that's out of scope.

---

## 14. Staying alive — heartbeats and dead sockets
...
6. **Shard `messages` by conversation.** Only once the message volume (~66 TB over 5 years, §15.4) outgrows one primary.
...
2. **V2 — registry + heartbeats.** Add the Redis registry (§7) and the heartbeat/TTL machinery (§14), so connection
...
4. **V4 — receipts (ticks).** Add the delivered/read cursors (§13) — viewport + debounce on the client, `GREATEST`cursor
   updates on the server, live push if the peer is online. Start with a synchronous cursor update; move to Redis-hot /
   Postgres-batched only when the receipt rate demands it.
5. **V5 — router tier.** Add routers (§9) so chat servers hand off via a small middle tier instead of a full mesh.
6. **V6 — read replicas.** Move history reads to replicas.
7. **V7 — shard messages by conversation.** Only when storage or write volume demands it (§15.4).
