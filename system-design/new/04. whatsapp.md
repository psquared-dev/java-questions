...
The `status` column on `local_messages` is what the tick UI reads: `pending` = clock icon, `sent` = one grey tick,
`delivered` = two grey, `read` = two blue. A freshly-typed message starts `pending` with `seq`, `id`, and
`conversation_id` all NULL — because the *server* assigns those — and the phone fills them in when the ack comes back.
This whole lifecycle is walked through end-to-end in §8.
