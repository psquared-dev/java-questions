...
sequenceDiagram
    ...
    PG -->> A: seq = 5
    A -->> Aisha: ack { client_msg_id: "abc", seq: 5 } (one grey tick)
...
*Fig. 2 — Store first, then deliver. The store step hands back `seq`, which flows to the sender's ack and, on delivery,
to the recipient.*

Walking through it at the level of every wake-up, read, lookup, and write:

**Step 1 — bytes arrive, epoll wakes a thread.** Aisha's phone writes down its socket to Server A. The bytes land in
Server A's receive buffer. The OS marks that socket "ready" and epoll wakes an I/O thread on Server A: "this Channel has
data."
...
Then it decodes the payload the phone sent:
...
-- (a) Find or create the conversation row for the pair (7, 9).
--     Stored smaller-id-first, so the pair is exactly one row (§4).
...
`seq = 5` — a collision. Wrapping (b) and (c) together means each message gets one clean, gap-free `seq`, and the
`UNIQUE(conversation_id, seq)` constraint (§4) is the final guard behind it.

**Step 4 — the store hands back `seq`, and the sender is acked with it.** The `RETURNING` in step (b) is where `seq = 5`
is produced. Server A sends it straight back to Aisha in her ack:

```text
ack → Aisha: { client_msg_id: "abc", seq: 5 }
```

Aisha's app now does three things with that `seq`, which is *why* the number is returned at all:

- **Confirms the message.** It matches the ack to her pending message by `client_msg_id = "abc"` and flips the bubble
  from "sending…" to a single grey tick ("stored").
- **Learns this message's position.** It now knows this message is `seq 5` in conversation 12, so it can order it
  correctly against everything else in that chat.
- **Advances her own cursor.** Her local `conversation 12 → last_received_seq` moves to `5`. (The sender has effectively
  "received" her own message — she won't re-fetch it on reconnect.)
...
**Step 6 — delivered tick.** Ben's phone receives the bytes (epoll wakes a thread on Ben's phone), stores the message
locally, sets its `conversation → last_received_seq = 5`, and sends a "delivered" ack back. When Server A gets it,
Aisha's app shows two ticks.

This same-server path is the fast, common case within one server. The next section handles the general case, where Ben
is on a *different* server — same steps, with a router hop added in the middle.
